...
func (c *criContainerdService) pullImage(ctx context.Context, ref string) (
	...
	p, err := content.ReadBlob(ctx, c.contentStoreService, digest)
	if err != nil {
		return "", "", fmt.Errorf("readblob failed for digest %q: %v", digest, err)
	}
	var manifest imagespec.Manifest
	if err := json.Unmarshal(p, &manifest); err != nil {
		return "", "", fmt.Errorf("unmarshal blob to manifest failed for digest %q: %v", digest, err)
	}
	...
	return configDesc.Digest, digest, nil
}
