...
// getImageInfo returns image chainID, compressed size and oci config. Note that getImageConfig
// assumes that the image has been pulled or it will return an error. Image reference passed-in
...
func (c *criContainerdService) getImageInfo(ctx context.Context, ref string) (
	...
	if desc.MediaType != imagespec.MediaTypeImageConfig && desc.MediaType != images.MediaTypeDockerSchema2Config {
		return "", 0, nil, fmt.Errorf("unknown image config media type %q", desc.MediaType)
	}
	...
	return chainID, size, &imageConfig, nil
}

// getRepoDigest returns image repoDigest of the named image reference.
func getRepoDigest(namedRef reference.Named, digest imagedigest.Digest) string {
	return namedRef.Name() + "@" + digest.String()
}
...
func (c *criContainerdService) localResolve(ctx context.Context, ref string) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("an error occurred when get image %q from containerd image store: %v",
